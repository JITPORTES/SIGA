...
<link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
	...
	<!-- ==== Info boxes ==== -->
	<div class="row" style="<?php if($Id_Menu==25){echo"display:none"; } ?>"></div>
<!-- <hr class="separation-line"> -->
        ...
        <div class="col-md-12 col-sm-6 col-xs-3">
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">SIGA: Inventario</h3>
            </div>
			...
			*/
			...
			$('input').each(function (i, e) {
				...
				case 'numempleadoresguardo':
					...
					break;
				case 'numempleadosolicitante':
					label = 'Número de Empleado Solicitante';
					break;	
	...
	*/
	...
	function usuarios_empleados_reubicacion(){
		...
		$.ajax({
			...
			success: function (datos) {
				var json = "";
					...
					if (json.totalCount > 0) {
						...
						usr_solicitante+='<option></option>';
						usr_solicitante+='<optgroup label="Usuario Solicitante">';

						for (var i = 0; i < json.totalCount; i++) {
							usr_solicitante+='<option value="'+json.data[i].Id_Usuario+'">'+json.data[i].No_Usuario+'-'+json.data[i].Nombre_Usuario+'</option>';
						}
						usr_solicitante+='</optgroup>';
						$('#numempleadoreubicacion').html(usr_baja);
						$('#numempleadoreubicacionsolicitante').html(usr_solicitante);

						$("#gifcargandoreubicacion").hide();
						$("#gifcargandoreubicacionsolicitante").hide();
						$("#numempleadoreubicacion").show();
						$("#numempleadoreubicacionsolicitante").show();

						$('#numempleadoreubicacion').selectize({
							//sortField: 'text'
						});
						$('#numempleadoreubicacionsolicitante').selectize({
							//sortField: 'text'
						});
					}
					else {
						...
						usr_baja+='<optgroup label="Usuario Responsable">';
						usr_baja+='</optgroup>';
						usr_solicitante+='<option>--Sin Resultados--</option>';
						usr_solicitante+='<optgroup label="Usuario Solicitante">';
						usr_solicitante+='</optgroup>';
						$('#numempleadoreubicacion').html(usr_baja);
						$("#numempleadoreubicacion").show();
			...
			error: function (objeto, quepaso, otroobj) {
				...
				$('#numempleadoreubicacionsolicitante').append($('<option>', { value: "-1" }).text("Sin resultados"));
			}
		});
	...
	$("#guardar").click(function () {
		...
		var Num_Empleado_Solicitante=$.trim($("#numempleadosolicitante").val());
		...
		if (Num_Empleado_Solicitante.length <= 0) {
			Agregar = false;
			mensaje_error += " -Falta agregar el Usuario Solicitante<br />";
			$("#numempleadosolicitante").focus();
		}else{
			if (Id_Activo.length <= 0){
				if(Num_Empleado_Solicitante==0){
					Agregar = false;
					mensaje_error += " -Agrega un Usuario Solicitante Diferente<br />";
					$("#numempleadosolicitante").focus();		
				}
			}
		}
	...
	function guardaDatosGenerales() {
		...
		ImporteSeguros=ImporteSeguros.replace(/\,/g,'');
			...
			var Num_Empleado_Solicitante=$.trim($("#numempleadosolicitante").val());
			if (Num_Empleado_Solicitante != "")
			strDatos += "&Num_Empleado_Solicitante="+Num_Empleado_Solicitante;
  ...
  return str.length < max ? pad("0"+str, max) : str;
	...
	function usuarios_empleados(){
		$.ajax({
			...
			success: function (datos) {
				var json = "";
					...
					if (json.totalCount > 0) {
						...
						var usuarios_solicitantes='';
						usuarios_solicitantes+='<option></option>';
						usuarios_solicitantes+='<optgroup label="Usuario Solicitante">';
						usuarios_solicitantes+='<option value="0">0-Sin definir</option>';
						if($("#idareasesion").val()==2){
							usuarios_solicitantes+='<option value="99999">99999-STOCK TI</option>';
						}

						for (var i = 0; i < json.totalCount; i++) {
							usuarios_solicitantes+='<option value="'+json.data[i].num_empleado.trim()+'">'+json.data[i].num_empleado.trim()+'-'+json.data[i].nombre_completo.trim()+'</option>';
						}
						usuarios_solicitantes+='</optgroup>';
						
						$('#numempleadosolicitante').html(usuarios_solicitantes);
						$("#gifcargandoaltausuarios").hide();
						$("#numempleadoresguardo").show();
						$("#numempleadosolicitante").show();

						$('#numempleadoresguardo').selectize({
							//sortField: 'text'
						});

						$('#numempleadosolicitante').selectize({
							//sortField: 'text'
						});
					}
					else {
						...
						$('#numempleadosolicitante').append($('<option>', { value: "" }).text("Sin resultados"));
					}
			...
			error: function (objeto, quepaso, otroobj) {
				...
				$('#numempleadosolicitante').append($('<option>', { value: "-1" }).text("Sin resultados"));
			}
		});
	}
    ...
    $("#generarreubicacion").click(function () {
		...
		var No_Empleado_Solicitante    = $.trim($("#numempleadoreubicacionsolicitante").val());
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		if (cmbestatusreubicacionguar == "-1") {
			Agregar = false;
	  ...
	  /* Obligatorios */
			...
			strDatos += "&No_Empleado_Solicitante="+No_Empleado_Solicitante;
...
if(Agregar){
	...
	$.ajax({
		type: "POST",
		url: "../class/admin/mesaDeAyuda/reubicacion/reubicacion.ajax.php",
		data: {
				accion:1,
				...
				No_Empleado_Solicitante: No_Empleado_Solicitante,
...
*/
	...
	limpiarcampos = function() {
		...
		if(Num_Empleado_Solicitante!=""){
			if(Num_Empleado_Solicitante.length > 0){
				var $select3_solic = $('#numempleadosolicitante').selectize({});
				var control3_solic = $select3_solic[0].selectize;
				control3_solic.clear();
			}
		}
	...
	limpiarcamposreubicacion=function(){
		...
		var Num_Empleado_Solicitante=$.trim($("#numempleadoreubicacionsolicitante").val());
		if(Num_Empleado_Solicitante!=""){
			if(Num_Empleado_Solicitante.length > 0){
				var $select4 = $('#numempleadoreubicacionsolicitante').selectize({});
				var control4 = $select4[0].selectize;
				control4.clear();
			}
		}
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data.totalCount > 0) {
						...
						var $select3_sol = $('#numempleadosolicitante').selectize({});
						var control3_sol = $select3_sol[0].selectize;
						control3_sol.addItem(data.data[0].Num_Empleado_Solicitante);
	...
	workflowreubicaciones=function(id){
		...
		$.ajax({
			...
			data: {
				...
				accion: "getworkflow_reubicacion"
			},
			dataType: "html",
			beforeSend: function (xhr) {
			},
			success: function (data) {
				...
				if (data.totalCount > 0) {
					...
					for (var i = 0; i < data.totalCount; i++) {
						...
						workflow += '<div class="process-step">';
						workflow += '    <button type="button" class="btn btn-info btn-circle '+colorwfl+'"><i class="fa fa-user fa-3x"></i></button>';
						workflow += '    <p><small>' + data.data[i].DescWorflow + '</small><br>';
						workflow += '    <small>' + data.data[i].Nombre + '</small><br>';
						workflow += '    <small><span><b>Fecha Flujo: </b> </span>' + data.data[i].FechaAceptado + '</small><br>';
						
						// Validación para mostrar leyenda si el correo está vacío o nulo
						if (!data.data[i].Correo || data.data[i].Correo.trim() === "") {
							workflow += '    <small><span><b>Correo:</b> </span>No cuenta con correo.</small><br>';
						} else {
				...
				$.ajax({
					...
					success: function (data) {
						...
						if (data.totalCount > 0) {
							...
							var $select_numempleadoreubicacionsolicitante = $('#numempleadoreubicacionsolicitante').selectize({});
							var control_numempleadoreubicaciosolicitante = $select_numempleadoreubicacionsolicitante[0].selectize;
							control_numempleadoreubicacionsolicitante.addItem(data.data[0].No_Empleado_Solicitante);
