			...
			try {
				...
				*/
				...
				$sqltotal = "SELECT S.Id_Activo,
					...
					(select top 1 Id_Activo from siga_workflow_alta_activo WA where WA.Id_Activo=S.Id_Activo and CveWorkflow=1) as WAId_Activo,
				...
				if($parametrosConsulta->estatus == "tablereubicacion") {
					$sqltotal .= "	(select top 1  Desc_Ubic_Prim from siga_cat_ubic_prim where (select top 1 HR.Id_Ubic_Prim from siga_historico_reubicacion HR where HR.Id_Activo_Reubicacion=SR.Id_Activo_Reubicacion)=siga_cat_ubic_prim.Id_Ubic_Prim) AS UbicacionPrimariaProcedencia,
									...
									(select WR.Id_Reubicacion_Activo from siga_workflow_reubicacion_activo WR where WR.Id_Reubicacion_Activo=SR.Id_Activo_Reubicacion and CveWorkflow=1) as WRId_Reubicacion_Activo,
				...
				*/
				...
				if($reader) {
					...
					while($row = $_proveedor->fetch_array($reader, 0)) {
							...
							/*Workflow Alta Activos*/
							"WAId_Activo" => $row["WAId_Activo"],

							/*Workflow reubicacion Activos*/
							"WRId_Reubicacion_Activo" => $row["WRId_Reubicacion_Activo"],
