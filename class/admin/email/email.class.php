...
class email extends conectar{
  ...
  public function envioEmail($tipo, $Correo_solicitante, $Id_Operacion, $motivo){

    $info = $this->datosGeneralesTicket($Id_Operacion);

    //		$valeMasivoEmail = 'itdeveloper@hospitalsatelite.com;';
		$Correo_solicitante = 'itdeveloper@hospitalsatelite.com;';
		
    if($tipo == 1){
      $titulo = "SIGA: Cancelación de Ticket. ".$Id_Operacion;      
      $texto = "";
      $liga = "";
      $strSubject = "SIGA: Cancelacion de Ticket. ".$Id_Operacion;
      ...
      $texto  = '<div style="text-align: center;">
                  ...
                  <b>Verificar que todos sus archivos importantes se guarden dentro de la carpeta de TRABAJO.</b>
                    <br><br>
                  <b>TODOS LOS EQUIPOS MENCIONADOS INCLUYEN CABLES DE CARGA, SE&Ntilde;AL Y ADAPTADORES (SEGUN SEA EL CASO), PARA SU CORRECTO FUNCIONAMIENTO.</b>
								  </div>';

      $liga = '<a href="https://apps2.hospitalsatelite.com/siga/controladores/activos/siga_vale_resguardo/reporte.php?Id_Vale_Resguardo='.$Id_Operacion.'">DESCARGA VALE</a>';
      $strSubject = 'SIGA: Vale de resguardo:'.$Id_Operacion;
    }
																							...
																							<td align="center" valign="top" class="specbundle3">
																								<div class="contentEditableContainer contentTextEditable">
																									<div class="contentEditable" style="color: #ffffff; font-size: 21px; line-height: 19px;">
																										<p><span>'.utf8_decode($titulo).'</span></p>
																									</div>
																								</div>
																							</td>
																						</tr>
	...
	';
		...
		$data = array('strPassword'=>'C68H17S49', 'strSubject'=>$strSubject, 'strTo'=>$info['Correo'], 'strHTMLBody'=>$infoUsuario, 'strBCC'=>'itdeveloper@hospitalsatelite.com;','strCc'=>'','system'=>'SIGA');
...
//cleon@hospitalsatelite.com; gchan@hospitalsatelite.com;
  ...
  public function datosGeneralesTicket($Id_solicitud){
    $pdo = conectar::ConexionGestafSiga();

      $sql = "SELECT TOP 1 Nombre_Usuario, Correo FROM siga_usuarios WHERE Id_Usuario= (SELECT Id_Usuario FROM siga_solicitud_tickets WHERE Id_Solicitud= $Id_solicitud)";
